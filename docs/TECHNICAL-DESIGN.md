...
passes, 397 unit tests across 31 files pass, the production build succeeds, and
...
| Suite | Runner | Scope |
|---|---|---|
| 397 unit tests, 31 files | vitest, `environment: "node"` | Pure server modules (heuristics, provider, prompt, label, eval verdict rules, authz, db-adapter, demo-guard, email, file-context, rate-limit, reset-guard, sentry-options, utils, validation, money, fx, months, search, concurrency, migration-ledger) and the server actions + seed against a real migrations-built SQLite (`src/test/action-harness.ts`) |
...
fixture — is red rather than a green wall of skips. The parrot payload is expected to fail
