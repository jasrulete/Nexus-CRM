...
| I5 | With no `GEMINI_API_KEY` and no `GROQ_API_KEY`, all three still work and the panel footer reads *"Generated by rule-based mode (no AI key configured)"*; with a key whose provider is failing it reads *"rule-based fallback (AI provider rate-limited)"*, *"(AI provider error)"* or *"(AI reply was not usable)"* |
