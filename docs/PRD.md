...
| G3 | AI features work with no API key at all | `generateText()` returns `null` with no key; every caller falls back to `src/lib/ai/heuristics.ts` and the UI labels the result "rule-based fallback" |
...
| I2 | A model reply that does not parse as JSON with a 0–100 integer score is **discarded** in favour of the deterministic heuristic — the score column can never hold garbage a model emitted |
...
| I5 | With no `GEMINI_API_KEY` and no `GROQ_API_KEY`, all three still work and the panel footer reads *"Generated by rule-based fallback (AI provider unavailable)"* |
...
9. **How should quota exhaustion be surfaced?** Falling back to heuristics is
   correct and labelled, but a visitor cannot tell whether they are seeing
   heuristics because no key is configured or because today's free-tier budget is
   spent. Those are different stories about the product.
