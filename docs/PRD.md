...
| I2 | A model reply that does not parse as JSON with a numeric 0–100 score (rounded to an integer) and a string reason is **discarded** (and, since 2026-09-13, so is a draft reply that is not shaped like an email — see I4) in favour of the deterministic heuristic, and the reason is recorded as `malformed` — the score column can never hold garbage a model emitted |
...
| I4 | **Draft email** returns a `Subject:` line and a body under ~130 words referencing the most relevant open deal. The shape is enforced: a model reply whose first non-blank line is not a `Subject:` line with subject text, or that has no body, is **discarded** — the next provider is tried, then the heuristic, labelled per I5. It is not persisted |
...
| I7 | CRM data reaches the model inside `<record>` tags, with a system preamble instructing the model to treat that content, and any pasted `<user-context>`, strictly as data |
...
| Build and test health | typecheck, eslint, 459 unit tests (coverage-gated), 45 e2e tests incl. axe scans, production build all green | GitHub Actions badge in the README |
...
459 unit tests cover library modules and the server actions (against a real
