...
describe("weightedValue", () => {
  it("weights each deal by its stage", () => {
    expect(
      weightedValue([
        { stage: "LEAD", value: 10_000 }, // 10% -> 1,000
        { stage: "NEGOTIATION", value: 20_000 }, // 75% -> 15,000
      ]),
    ).toBe(16_000);
  });

  it("counts a won deal in full and a lost deal not at all", () => {
    expect(weightedValue([{ stage: "WON", value: 5_000 }])).toBe(5_000);
    expect(weightedValue([{ stage: "LOST", value: 5_000 }])).toBe(0);
  });
  ...
  it("ignores a stage that is not one of the known six", () => {
    ...
    expect(weightedValue([{ stage: "PROPOSL", value: 10_000 }])).toBe(0);
  });

  it("rounds to whole currency units, matching how Deal.value is stored", () => {
    // 3333 * 0.1 = 333.3
    expect(weightedValue([{ stage: "LEAD", value: 3_333 }])).toBe(333);
    expect(Number.isInteger(weightedValue([{ stage: "PROPOSAL", value: 999 }]))).toBe(
      true,
    );
  });
