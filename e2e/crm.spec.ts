...
// Scoring is the one AI action whose result is not rendered as a block, so its
// provenance gets a line of its own under the score. With the keys blanked the
// reason is "no API key configured" - not "provider unavailable".
test("scoring a contact says what produced the score", async ({ page }) => {
  await page.goto("/contacts");
  ...
  // Either shape the rule produces: "Rule-based score: ..." or "... from profile completeness."
  await expect(page.getByText(/^Rule-based score/)).toBeVisible({ timeout: 30_000 });
  if (!process.env.E2E_LIVE_AI) {
    await expect(
      page.getByText("Scored by rule-based mode (no API key configured)"),
    ).toBeVisible();
  }
});
...
test("a drafted follow-up can be sent to yourself and lands in the feed", async ({
  ...
  if (!process.env.E2E_LIVE_AI) {
    await expect(
      page.getByText("Generated by rule-based mode (no API key configured)"),
    ).toBeVisible();
  }
